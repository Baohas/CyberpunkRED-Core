...
const Attackable = function Attackable() {
  ...
  this._createDamageRoll = function _createDamageRoll(type, actor) {
    ...
    // Resolve the damage-critical config: ammo "set" override beats the weapon's own damageCrit, which
    // defaults to RAW (2+ dice at max face → +5). A `cprDamageCrit` hook lets modules tweak it for
    // homebrew rules the datamodel knobs can't express.
